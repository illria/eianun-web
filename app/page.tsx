...
export default function Home() {
  ...
  return (
    <main className={`eianun-site ${dark ? "dark" : ""}`}>
      ...
      <section className="oc-section featured-section" id="featured"><div className="oc-section-head"><div><span className="oc-label">FEATURED / FIELD NOTES</span><h2>泥伏雷出品，<br /><em>独立专题。</em></h2></div><p>把一个主题讲完整，<br />再回到工具和路径。</p></div><div className="featured-grid"><button onClick={() => { setCategory("冷钱包"); jump("tool-library"); }}><span>▣</span><small>自用指南</small><h3>硬件冷钱包全品牌横评</h3><p>OneKey · Ledger · SafePal · Tangem，先看保管逻辑，再选设备。</p><b>进入专题　↗</b></button><button onClick={() => { setCategory("券商"); jump("tool-library"); }}><span>↗</span><small>新玩法</small><h3>链上美股与全球资产</h3><p>把开户、入金和资产配置拆开，理解每一个入口的边界。</p><b>进入专题　↗</b></button><button onClick={() => { setCategory("加密交易所"); jump("tool-library"); }}><span>₿</span><small>新手必看</small><h3>购买比特币入门路径</h3><p>从交易所、第一笔买入到转出保管，按步骤检查，不跳关。</p><b>进入专题　↗</b></button></div></section>
